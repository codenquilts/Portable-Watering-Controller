...
WifiModeState wifiBegin(DeviceCfg& cfg) {
  ...
  if (wifiIsSetupMode()) {
    ...
    if (ok && WiFi.status() == WL_CONNECTED) {
      ...
      wifiClearSetupModeFlag();
      Serial.println("WiFi setup complete; rebooting into normal STA mode...");
      delay(500);
      ESP.restart();
    }
...
std::vector<String> wifiScanNetworks() {
  ...
  networks.reserve(count > 0 ? count : 0);
...
bool wifiTryConnect(const String& ssid, const String& pass, const String& deviceName) {
  ...
  g_state = WifiModeState::STA_CONNECTED;
  timeOnWifiConnected();
  mdnsStart(deviceName);
  Serial.printf("WiFi connected via built-in manager, IP=%s\n", WiFi.localIP().toString().c_str());
  return true;
}
