...
server.on("/api/status", HTTP_GET, [](AsyncWebServerRequest* req){
  ...
  m["start_hhmm"] = g_cfg->morning.startHHMM;
  m["run_min"]    = g_cfg->morning.runMin;
  m["enabled"]    = g_cfg->morning.enabled;

  JsonObject e = doc.createNestedObject("evening");
  e["start_hhmm"] = g_cfg->evening.startHHMM;
  e["run_min"]    = g_cfg->evening.runMin;
  e["enabled"]    = g_cfg->evening.enabled;
  ...
  // ----------------------------
  server.on("/api/schedule/morning", HTTP_POST,
  ...
  // ----------------------------
  server.on("/api/schedule/evening", HTTP_POST,
    [](AsyncWebServerRequest* req){},
    nullptr,
    [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t){
  ...
  server.on("/api/pump", HTTP_POST,
    ...
    [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t){
      ...
      if (state) {
        ...
        pumpStartForMinutes(*g_st, RUN_MAX_MINUTES);
      } else {
        pumpStop(*g_st);
      }
  ...
  // ----------------------------
  server.on("/api/runNow", HTTP_POST,
    [](AsyncWebServerRequest* req){},
    nullptr,
    [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t){
      ...
      if (which == "Morning") pumpStartForMinutes(*g_st, g_cfg->morning.runMin);
      else                    pumpStartForMinutes(*g_st, g_cfg->evening.runMin);
  ...
  // Tank reset
  // ----------------------------
  server.on("/api/tankReset", HTTP_POST,
    [](AsyncWebServerRequest* req){},
    nullptr,
  ...
  // ----------------------------
  server.on("/api/wifi/setup", HTTP_POST,
    [](AsyncWebServerRequest* req){},
    nullptr,
    [](AsyncWebServerRequest* req, uint8_t*, size_t, size_t, size_t){
